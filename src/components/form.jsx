    ...
    <div>
      ...
      <div style={{ position:"relative" }}>
        <input id={fieldId} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} type={isPw&&!showPw?"password":"text"} onKeyDown={onKeyDown} inputMode={inputMode}
          style={{ width:"100%", padding:"12px 14px", paddingRight:isPw?42:14, borderRadius: R.md, border:`1.5px solid ${borderColor}`, background:hasError?C.errPale+"40":C.w, color:C.t1, fontSize:17.6, fontFamily:"inherit", outline:"none", boxSizing:"border-box" }}
    ...
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <input type="number" inputMode="decimal" value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} min={min} max={max} step={step}
          style={{ width:80, padding:"10px 4px", borderRadius: R.md, border:`1.5px solid ${C.b1}`, background:C.w, color:C.t1, fontSize:17.6, fontFamily:"inherit", outline:"none", boxSizing:"border-box", textAlign:"center" }}
...
export function Sec({ label, complete, summary, children, isExpanded, onFocus, secRef, incomplete, highlight, disabled }) {
  ...
  return (
    <div ref={secRef} style={{ transition:"all 0.3s ease" }}>
      {!isExpanded ? (
        <>
          <button type="button" onClick={handleClick} style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"12px 14px", borderRadius: R.lg, border:`1.5px solid ${complete?C.ok+'40':(incomplete||hl)?C.acc+'60':C.b1}`, background:complete?`${C.ok}08`:(incomplete||hl)?`${C.acc}08`:C.w, cursor:disabled?"not-allowed":"pointer", opacity:disabled?0.5:1, fontFamily:"inherit", textAlign:"left", transition:"all 0.2s ease" }} tabIndex={0}>
            {complete ? Ic.chk(C.ok,16) : (incomplete||hl) ? <span style={{width:10,height:10,borderRadius: R.sm,background:C.acc,display:"inline-block",animation:"dotPulse 1.5s ease-in-out infinite",flexShrink:0}}/> : <span style={{width:10,height:10,borderRadius: R.sm,border:`2px solid ${C.b1}`,display:"inline-block",flexShrink:0}}/>}
          ...
          {showWarn && <div style={{marginTop:6,padding:"6px 12px",borderRadius: R.md,background:C.acc+'18',border:`1px solid ${C.acc}40`,fontSize:12.1,fontWeight:600,color:C.acc,display:"flex",alignItems:"center",gap:6,animation:"fadeIn 0.2s ease"}}>{Ic.warn(C.acc,14)} Completá los campos anteriores primero</div>}
        </>
      ) : (
        <div>
          <div style={{fontSize:12.1,fontWeight:700,color:C.pri,textTransform:"uppercase",letterSpacing:0.5,marginBottom:12,display:"flex",alignItems:"center",gap:6}}><span style={{width:8,height:8,borderRadius: R.xs,background:C.pri,display:"inline-block"}}/>{label}</div>
          {children}
        </div>
      )}
    </div>
