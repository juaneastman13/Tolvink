...
export default function DriverQueueModal({ driverId, driverName, onClose }) {
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div style={{ fontSize: 18.7, fontWeight: 700, marginBottom: 4 }}>Cola del chofer</div>
      <div style={{ fontSize: 13.2, color: C.t2, marginBottom: 16 }}>{driverName || "Chofer"}</div>

      {loading && <div style={{ fontSize: 13.2, color: C.t3, padding: 20, textAlign: "center" }}>Cargando cola...</div>}

      {!loading && queue.length === 0 && <div style={{ fontSize: 13.2, color: C.t3, padding: 20, textAlign: "center" }}>Sin fletes en cola</div>}
