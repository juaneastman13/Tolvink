...
export default function ReasonModal({ title, freight, btnLabel, btnType="err", onClose, onConfirm }) {
  ...
  return (
    <ModalOverlay onClose={onClose} loading={loading} closing={closing} closingText={closingText} quick>
      <div style={{fontSize:18.7,fontWeight:700,marginBottom:4,color:btnType==="err"?C.err:C.t1}}>{title} · {freight.code}</div>
      <div style={{fontSize:13.2,color:C.t2,marginBottom:18}}>{freight.grain} · {freight.tons}tn</div>
      ...
      <textarea value={reason} onChange={e=>setReason(e.target.value)} placeholder="Describí el motivo..." rows={3} style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`1.5px solid ${C.b1}`,background:C.w,color:C.t1,fontSize:14.3,fontFamily:"inherit",outline:"none",resize:"none",boxSizing:"border-box",marginBottom:16}}/>
