...
export default function MenuScreen({ user, perms, onLogout, onNav, isDesktop, onSwitchCompany, onRefresh, simpleMode, onToggleSimple }) {
  ...
  if (!isChofer) {
    ...
    if(!isDesktop) {
      if(isGerente || isPlantUser) mgmtItems.push({k:"linked",l:"Empresas vinculadas",ic:Ic.plant(C.t3,18),c:C.t3});
    }
  }
  ...
  const menuItem = (m, i, arr) => (
    <button key={m.k} onClick={()=>onNav(m.k)} aria-label={m.l} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"14px 14px",minHeight:52,background:"none",border:"none",borderTop:i>0?`1px solid ${C.b2}`:"none",cursor:"pointer",fontFamily:"inherit",textAlign:"left",transition:"background 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background=C.priGhost} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
      <div style={{width:36,height:36,borderRadius: R.md,background:`${m.c}12`,display:"flex",alignItems:"center",justifyContent:"center"}}>{m.ic}</div>
  ...
  const profileSection = (
    <div style={{background:C.w,border:`1px solid ${C.b1}`,borderRadius: R.lg,padding:16,boxShadow:C.sh}}>
      ...
      <div style={{marginBottom:12}}>
        ...
        {isDesktop ? (
        <div style={{border:`1px solid ${C.b2}`,borderRadius: R.md,overflow:"hidden"}}>
                  ...
                  <tr onClick={()=>setSwitching(isExp?null:"exp_"+c.companyId)} style={{cursor:"pointer",borderTop:i>0?`1px solid ${C.b2}`:"none",background:isExp?`${C.pri}06`:"transparent",transition:"background 0.15s"}}>
                    <td style={{padding:"8px 10px",fontWeight:600,color:C.t1,fontSize:13.2}}>
                      <div style={{display:"flex",alignItems:"center",gap:5}}>
                        {companies.length>1 && <div style={{width:18,height:18,borderRadius: R.md,border:`2px solid ${isActive?C.pri:C.b2}`,background:isActive?C.pri:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer"}} onClick={e=>{e.stopPropagation();if(!isActive)handleSwitch(c.companyId);}}>{isActive&&<div style={{width:8,height:8,borderRadius: R.xs,background:C.w}}/>}</div>}
                  ...
                  {isExp && <tr><td colSpan={3} style={{padding:"6px 10px 10px",background:`${C.pri}04`,borderTop:`1px dashed ${C.b2}`}}>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                      {(()=>{
                        ...
                        return p.map((pp,j)=><div key={j} style={{display:"flex",alignItems:"center",gap:3,fontSize:12.1,color:C.t1,padding:"2px 7px",background:C.w,borderRadius: R.sm,border:`1px solid ${C.b2}`}}>{Ic.chk(C.pri,10)} {pp}</div>);
                      })()}
                    </div>
                  </td></tr>}
                </Fragment>);
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {companies.map((c,i)=>{
            ...
            const roleLabel = c.role === "chofer" ? "Chofer" : c.role === "gerente" || c.effectiveRole === "admin" ? "Gerente" : "Operario";
            const isExp = switching === "exp_"+c.companyId;
            return (<div key={c.companyId||i} style={{border:`1.5px solid ${isActive?C.pri:C.b2}`,borderRadius: R.md,overflow:"hidden",background:isActive?`${C.pri}04`:C.w,transition:"all 0.15s"}}>
              <button onClick={()=>{if(companies.length>1&&!isActive)handleSwitch(c.companyId);setSwitching(isExp?null:"exp_"+c.companyId);}} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"12px 14px",minHeight:52,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
                {companies.length>1 && <div style={{width:20,height:20,borderRadius: R.md,border:`2px solid ${isActive?C.pri:C.b2}`,background:isActive?C.pri:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{isActive&&<div style={{width:8,height:8,borderRadius: R.xs,background:C.w}}/>}</div>}
  ...
  const leftPanel = (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {/* Management items */}
      {mgmtItems.length>0 && (
        <div style={{background:C.w,border:`1px solid ${C.b1}`,borderRadius: R.lg,padding:4,boxShadow:C.sh}}>
      ...
      {/* PWA install banner */}
      {canInstall && (
        <button onClick={() => window.installPWA?.()} style={{ width:"100%", padding:"14px 16px", borderRadius: R.lg, border:`1.5px solid ${C.pri}`, background:C.priPale, color:C.pri, fontSize:14.3, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:8, boxShadow:C.sh }}>
