...
export default function CompanyHeaderPicker({ user, onSwitch }) {
  ...
  return (
    <div ref={ref} style={{position:"relative",display:"flex",flexDirection:"column",alignItems:"flex-end"}}>
      <div style={{fontSize:11,color:C.t3,textTransform:"capitalize",marginBottom:2}}>{dateLabel} · {timeLabel}</div>
      <button onClick={()=>setOpen(!open)} style={{background:"none",border:`1px solid ${C.b2}`,borderRadius: R.md,padding:"5px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontFamily:"inherit"}}>
        <span style={{width:8,height:8,borderRadius: R.xs,background:tColor,flexShrink:0}}/>
        <span style={{fontSize:14.3,fontWeight:700,color:C.t1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:180}}>{activeName}</span>
        <span style={{fontSize:11,color:C.t3,flexShrink:0}}>▼</span>
      </button>
      {open && (
        <div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:C.w,border:`1px solid ${C.b1}`,borderRadius: R.md,boxShadow:C.shMd,padding:4,zIndex:100,minWidth:180,maxWidth:280}}>
          {companies.map(c=>{
            const isAct = c.companyId===user.activeCompanyId;
            return <button key={c.companyId} onClick={()=>{setOpen(false);if(c.companyId!==user.activeCompanyId)onSwitch(c.companyId);}} style={{display:"flex",alignItems:"center",gap:6,width:"100%",padding:"8px 10px",background:isAct?`${C.pri}08`:"transparent",border:"none",borderRadius: R.md,cursor:isAct?"default":"pointer",fontFamily:"inherit",textAlign:"left"}}>
              <span style={{width:6,height:6,borderRadius: R.xs,background:TYPE_C[c.companyType]||C.t2,flexShrink:0}}/>
              ...
              <span style={{fontSize:9.9,color:C.t3}}>{TYPE_L[c.companyType]||""}</span>
