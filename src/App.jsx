...
export default function Tolvink() {
  ...
  // Zustand UI store — shallow selectors to prevent full re-render on every state change
  const { modal, toast, mapFocus, listView, submitting, submitDone, actionLoading, notifOpen, chatConvId, duplicateData, editData } = useUIStore(
    useShallow(s => ({ modal:s.modal, toast:s.toast, mapFocus:s.mapFocus, listView:s.listView, submitting:s.submitting, submitDone:s.submitDone, actionLoading:s.actionLoading, notifOpen:s.notifOpen, chatConvId:s.chatConvId, duplicateData:s.duplicateData, editData:s.editData }))
  );
  // Actions — stable refs, never trigger re-renders
  const { setModal, setToast, show: uiShow, setMapFocus, setListView, setSubmitting, setSubmitDone, setActionLoading, setNotifOpen, setChatConvId, setDuplicateData, setEditData, goToMap } = useUIStore(
    useShallow(s => ({ setModal:s.setModal, setToast:s.setToast, show:s.show, setMapFocus:s.setMapFocus, setListView:s.setListView, setSubmitting:s.setSubmitting, setSubmitDone:s.setSubmitDone, setActionLoading:s.setActionLoading, setNotifOpen:s.setNotifOpen, setChatConvId:s.setChatConvId, setDuplicateData:s.setDuplicateData, setEditData:s.setEditData, goToMap:s.goToMap }))
  );
  ...
  return (
    <div className="tv-shell" style={{height:"100dvh",background:C.bg,color:C.t1,fontFamily:FONT,display:"flex",flexDirection:isDesktop?"row":"column",width:"100%",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0, position:"relative", zIndex:1 }}>
        ...
        <div className="tv-page" style={{flex:1,display:"flex",flexDirection:"column",minHeight:0}}>
