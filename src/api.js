...
export async function apiReorderTruckQueue(truckId, orderedAssignmentIds) { return api(`/freights/trucks/${truckId}/queue/reorder`,{method:'PATCH',body:{orderedAssignmentIds}}); }
...
export async function apiGetCompanyAccess(companyId, type) { if(!companyId) return []; const p=new URLSearchParams(); if(type)p.set('type',type); const q=p.toString(); return api(`/company-access/${companyId}${q?`?${q}`:''}`); }
export async function apiGetMyAccess() { return api('/company-access/my-access'); }
export async function apiGetLinkedStats(companyId) { if(!companyId) return {}; return api(`/company-access/stats/${companyId}`); }
export async function apiGetUnifiedAccess(companyId) { if(!companyId) return []; return api(`/company-access/unified/${companyId}`); }
