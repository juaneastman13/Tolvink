...
export function Sec({ label, complete, summary, children, isExpanded, onFocus, secRef, incomplete, highlight }) {
  ...
  return (
    <div ref={secRef} style={{ transition:"all 0.3s ease" }}>
      {!isExpanded ? (
        <button type="button" onClick={onFocus} style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"12px 14px", borderRadius:12, border:`1.5px solid ${complete?C.ok+'40':(incomplete||hl)?C.acc+'60':C.b1}`, background:complete?`${C.ok}08`:(incomplete||hl)?`${C.acc}08`:C.w, cursor:"pointer", fontFamily:"inherit", textAlign:"left", transition:"all 0.2s ease" }} tabIndex={0}>
