...
export function Sidebar({ active, onChange, unread=0, pendingCount=0, notifCount=0, canRequest=false, onNew, activeCompany, companies=[], onSwitchCompany, simpleMode=false, onToggleSimple }) {
  ...
  return (
    <div style={{ width:220, minWidth:220, height:"100%", background:C.w, borderRight:`1px solid ${C.b2}`, display:"flex", flexDirection:"column", flexShrink:0, overflow:"hidden" }}>
      {/* Logo */}
      <div style={{ padding:"16px 20px 14px", borderBottom:`1px solid ${C.b2}` }}>
        <div style={{ display:"inline-flex", alignItems:"flex-start" }}>
          <span style={{ fontSize:38, fontWeight:800, color:C.pri, letterSpacing:-1.5, lineHeight:1 }}>tolvink</span>
          <span style={{ width:10, height:10, borderRadius:5, background:C.acc, display:"inline-block", marginLeft:3, marginTop:2, animation:"dotPulse 1.5s ease-in-out infinite" }}></span>
        </div>
      </div>
